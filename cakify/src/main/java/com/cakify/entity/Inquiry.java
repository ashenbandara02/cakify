 ...
 * - Database indexes on frequently queried columns (status, category_id, created_at, email)
 * - Improves query performance for filtering, sorting, and searching
 */
@Entity
@Table(name = "inquiries", indexes = {
    @Index(name = "idx_inquiry_status", columnList = "status"),
    @Index(name = "idx_inquiry_category", columnList = "category_id"),
